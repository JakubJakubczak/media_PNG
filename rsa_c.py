...
def generate_rsa_keypair(bits=2048):
    ...
    d = inverse(e, phi)
    privkeys = (n,e,d)
    priv_key = RSA.construct(privkeys)
...
def png_rsa_encryption(file_path, pub_key, method=0,compressed=1):
    output_path = file_path.replace('.png', 'rsa_encryption.png')

    #pub_key = rsa.PublicKey.load_pkcs1(publi_key)
    #pub_key = RSA.import_key(publi_key,passphrase=None)
    #pub_key = RSA.construct(publi_key,True)
        ...
        while True:
            ...
            if chunk_type == b'IDAT':
                ...
                if skip == 0:
                    print(f"IDAT CHUNK, ENCRYPTING:")           ########################################
                    if compressed == 1:
                        encrypted_test = PKCS1_OAEP.new(pub_key)
                        csize = pub_key.size_in_bytes() - 2 * encrypted_test._hashObj.digest_size - 2
                        encrypted_chunks = []
                        enchunk = PKCS1_OAEP.new(pub_key)
                        for i in range(0, len(idat_data),csize):
                            plainte = idat_data[i:i + csize]
                            chunk = enchunk.encrypt(plainte)
                        ...
                        decompressed = zlib.decompress(idat_data)
                        decompressed = decompressed.encode('ascii')

                        encrypted_test = PKCS1_OAEP.new(pub_key)
                        csize = pub_key.size_in_bytes() - 2 * encrypted_test._hashObj.digest_size - 2
                        encrypted_chunks = []
                        enchunk = PKCS1_OAEP.new(pub_key)
                        for i in range(0, len(decompressed),csize):
                            plainte = decompressed[i:i + csize]
                            chunk = enchunk.encrypt(plainte)
                            encrypted_chunks.append(chunk)
                        encrypted_data = b''.join(encrypted_chunks)
...
def png_rsa_decryption(file_path, priv_key,method=0,compressed=1,IV=0):
    output_path = file_path.replace('.png', 'rsa_decryption.png')
        ...
        while True:
            ...
            if chunk_type == b'IDAT':
                if skip == 0:
                    ...
                    if compressed == 1:
                        #decrypt
                        csize = priv_key.size_in_bytes()
                        decrypted_chunks = []
                        enchunk = PKCS1_OAEP.new(priv_key)
                        for i in range(0, len(idat_data),csize):
                            plainte = idat_data[i:i + csize]
                            chunk = enchunk.decrypt(plainte)
